...
export class RectShape implements IShape {
  ...
  public paint = (
    ...
    if (selected) {
      ...
      const { metadata } = this.annotationData;
      if (metadata?.ghost === "Yes" && metadata?.value) {
        canvas2D.font = `${fontSize}px ${fontFamily}`;
        const metrics = canvas2D.measureText(metadata.value);
        canvas2D.save();
        ...
        canvas2D.fillText(metadata.value, x + padding, y + padding);
      }
    }
    canvas2D.restore();
  ...
  public setMetadata = (metadata: IMetadata) => {
    this.annotationData.metadata = metadata;
  };
  ...
  public equal = (data: IAnnotation) => {
    return (
      ...
      data.metadata?.check === this.annotationData.metadata?.check &&
      data.metadata?.connect === this.annotationData.metadata?.connect &&
      data.metadata?.entry === this.annotationData.metadata?.entry &&
      data.metadata?.force === this.annotationData.metadata?.force &&
      data.metadata?.ghost === this.annotationData.metadata?.ghost &&
      data.metadata?.valid === this.annotationData.metadata?.valid &&
      data.metadata?.group === this.annotationData.metadata?.group &&
      data.metadata?.rotate === this.annotationData.metadata?.rotate &&
      data.metadata?.text === this.annotationData.metadata?.text &&
      data.metadata?.type === this.annotationData.metadata?.type &&
      data.metadata?.value === this.annotationData.metadata?.value
    );
  };
}

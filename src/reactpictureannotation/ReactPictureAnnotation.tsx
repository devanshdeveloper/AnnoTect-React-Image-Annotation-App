...
interface IReactPictureAnnotationProps {
  ...
  inputElement: (
    ...
    onDelete: () => void,
    showInput: boolean,
    changeSelected: (id: string | null) => void
  ) => React.ReactElement;
}
...
export default class ReactPictureAnnotation extends React.Component<IReactPictureAnnotationProps> {
  ...
  public render() {
    ...
    const { showInput, formData } = this.state;
    return (
      <div className="rp-stage">
        ...
        {showInput && (
          <div
            ...
            style={{ bottom: "calc(20px - (100vh))", right: "20px" }}
          >
            {inputElement(
              formData,
              this.onInputMetadataChange,
              this.onDelete,
              showInput,
              (id) => {
                this.selectedId = id;
    ...
    if (this.canvas2D && this.canvasRef.current) {
      ...
      for (const item of this.shapes) {
        ...
        if (isSelected) {
          ...
          this.setState({
            showInput: true,
            formData: item.getAnnotationData().metadata || getDefaultMetadata(),
          });
        }
      }
    ...
    if (annotationData) {
      ...
      if (annotationData.length !== this.shapes.length) {
        ...
        for (const annotationDataItem of annotationData) {
          const targetShape = this.shapes.find((item) => {
  ...
  private onInputMetadataChange = (metadata: IMetadata) => {
    const selectedShapeIndex = this.shapes.findIndex(
      (item) => item.getAnnotationData().id === this.selectedId
    );
    this.shapes[selectedShapeIndex].setMetadata(metadata);
    this.setState({ formData: metadata });
  };
